...
function EditMemesPage() {
    ...
    useEffect(() => {
        ...
        try {
            const response = await fetch('/api/memes'); // Replace with your API endpoint
...
const handleNewItemClick = () => {
    setNewItem({ topline: '', bottomline: '', extra: '', flagged: 0 }); // Reset the new item form
};
...
const handleSaveClick = async () => {
    try {
        const response = await fetch('/api/meme/update', { // Replace with your update API endpoint
            method: 'POST',
            headers: {
...
const handleCreateClick = async () => {
    try {
        const dataToSend = {
            ...newItem,
            flagged: newItem.flagged ? 1 : 0, // Convert boolean to 0 or 1
        };

        const response = await fetch('/api/meme/new', { // Replace with your create API endpoint
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
        ...
        if (response.ok) {
            ...
            setData([...data, createdItem]); // Add the new item to the data array
            setNewItem({ name: '', value: '' }); // Reset the form
        } else {
        ...
        <Card>
            <Card.Body>
                ...
                <Form.Check
                    ...
                    className="mx-2"
                />
                {/* <Form.Control type="number" max="1" min="0" name="flagged" value={newItem.flagged} onChange={handleNewItemChange} /> */}
